...
import { FormsModule } from '@angular/forms';
import { forkJoin } from 'rxjs';
import { Category, Income, IncomeRequest, RecurringRequest } from '../../core/models/api.models';
import { CategoriesApiService } from '../../core/services/categories-api.service';
import { IncomeApiService } from '../../core/services/income-api.service';
...
export class IncomeComponent implements OnInit {
  ...
  errorMessage = '';
  editingIncomeId: number | null = null;
  private editingOriginalIsRecurring = false;
  ...
  openCreate(): void {
    this.editingIncomeId = null;
    this.editingOriginalIsRecurring = false;
    this.form = this.emptyForm();
    this.modal.open('addIncome');
  }
  ...
  openEdit(income: Income): void {
    this.editingIncomeId = income.incomeId;
    this.editingOriginalIsRecurring = income.recurring;
    this.form = {
      title: income.title,
      amount: income.amount,
      date: income.date,
  ...
  saveIncome(): void {
    ...
    const isNew = !this.editingIncomeId;
    ...
    request.subscribe({
      next: () => {
        // Auto-create recurring template if marked recurring and it's new,
        // or if recurring was just toggled ON during an edit
        const shouldCreateRecurring = this.form.isRecurring &&
          (isNew || !this.editingOriginalIsRecurring);
        ...
        if (shouldCreateRecurring) {
          const recurringPayload: RecurringRequest = {
            ...
            amount: this.form.amount!,
            type: 'INCOME',
            frequency: this.form.recurrencePeriod ?? 'MONTHLY',
          ...
          this.recurringApi.create(recurringPayload).subscribe({ error: () => {} });
        }
