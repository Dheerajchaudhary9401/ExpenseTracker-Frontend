...
import { Budget, BudgetProgress, BudgetRequest, Category, Expense } from '../../core/models/api.models';
...
export class BudgetsComponent implements OnInit {
  ...
  get totalSpent(): number {
    ...
    return this.budgets.reduce((sum, budget) => sum + (budget.spentAmount ?? 0), 0);
  ...
  saveBudget(): void {
    ...
    request.subscribe({
      next: (response) => {
        ...
        this.form = this.emptyForm();
        this.errorMessage = '';
        // Sync existing expenses for this category into the new budget
        if (this.form.categoryId && !this.editingBudgetId) {
          const userId = this.session.userId()!;
          this.expensesApi.listByUser(userId).subscribe({
            next: (expenses: Expense[]) => {
              const total = expenses
                .filter((e: Expense) => e.categoryId === this.form.categoryId)
                .reduce((sum: number, e: Expense) => sum + e.amount, 0);
              if (total > 0) {
                this.budgetsApi.updateSpent(response.budgetId, total).subscribe();
              }
            }
          });
        }
