...
// import { buildCategoryBreakdown, formatCurrency, monthName, toTimelineTransactions } from '../../core/utils/formatters';
...
//       return forkJoin({
//         income: this.incomeApi.listByMonth(userId, target.getFullYear(), target.getMonth() + 1),
//         expense: this.expensesApi.listByMonth(userId, target.getFullYear(), target.getMonth() + 1)
//         // income: this.incomeApi.listByMonth(userId, now.getFullYear(), now.getMonth() + 1),
//         // expense: this.expensesApi.listByMonth(userId, now.getFullYear(), now.getMonth() + 1)
//       }).pipe(catchError(() => of({ income: [], expense: [] })));
//     });

//     forkJoin({
//       currentIncome: this.incomeApi.listByMonth(userId, now.getFullYear(), now.getMonth() + 1).pipe(catchError(() => of([] as Income[]))),
//       currentExpense: this.expensesApi.listByMonth(userId, now.getFullYear(), now.getMonth() + 1).pipe(catchError(() => of([] as Expense[]))),
...
//         this.monthlyTrend = (trends as Array<{ income: Income[]; expense: Expense[] }>).map((entry, index) => ({
//           label: monthName((now.getMonth() - (5 - index) + 12) % 12),
//           income: (entry?.income ?? []).reduce((sum: number, item: Income) => sum + item.amount, 0),
//           expense: (entry?.expense ?? []).reduce((sum: number, item: Expense) => sum + item.amount, 0)
//         }));
...
export class DashboardComponent implements AfterViewInit {
  ...
  private loadData(): void {
    ...
    forkJoin({
      ...
      categories: this.categoriesApi.listByUser(userId).pipe(catchError(() => of([] as Category[]))),
      budgets: this.budgetsApi.listActive(userId).pipe(catchError(() => of([] as Budget[]))),
      recurring: this.recurringApi.listUpcoming(userId).pipe(catchError(() => of([] as RecurringTransaction[]))),
      notifications: this.notificationsApi.listByRecipient(userId).pipe(catchError(() => of([]))),
      trends: forkJoin(monthRequests),
    }).subscribe({
      next: ({ currentIncome, currentExpense, categories, budgets, recurring, trends }) => {
        ...
        this.recentTransactions = toTimelineTransactions(currentExpense, currentIncome, categories).slice(0, 5);
        this.categoryBreakdown = buildCategoryBreakdown(currentExpense, categories).slice(0, 6);
        this.recurringItems = recurring
          .sort((a, b) => {
            const dateA = new Date(a.nextDueDate || a.startDate).getTime();
            const dateB = new Date(b.nextDueDate || b.startDate).getTime();
            return dateA - dateB;
          })
          .slice(0, 5);
        this.monthlyTrend = (trends as Array<{ income: Income[]; expense: Expense[] }>).map((entry, index) => ({
