...
export class AnalyticsComponent implements AfterViewInit {
  ...
  private loadData(): void {
    ...
    const monthRequests = Array.from({ length: 12 }, (_, index) => {
      ...
      return forkJoin({
        income: this.incomeApi.listByMonth(userId, target.getFullYear(), target.getMonth() + 1).pipe(catchError(() => of([] as Income[]))),
        expenses: this.expensesApi.listByMonth(userId, target.getFullYear(), target.getMonth() + 1).pipe(catchError(() => of([] as Expense[])))
      });
    });

    forkJoin({
      months: forkJoin(monthRequests),
      currentExpenses: this.expensesApi.listByMonth(userId, now.getFullYear(), now.getMonth() + 1).pipe(catchError(() => of([] as Expense[]))),
